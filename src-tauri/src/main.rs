...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Alarma nativa: la página avisa cuándo termina el bloque y un hilo
            // nativo (que macOS nunca congela) hace sonar el cucú a esa hora,
            // aunque la tarjeta lleve horas oculta.
            let alarma: Arc<Mutex<u64>> = Arc::new(Mutex::new(0));
            app.listen("cuckoo-fin", move |event| {
                let fin_ms: u64 = serde_json::from_str::<String>(event.payload())
                    .ok()
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(0);
                let mut gen = alarma.lock().unwrap();
                *gen += 1;
                let mi_gen = *gen;
                drop(gen);
                if fin_ms == 0 {
                    return; // pausa o reinicio: se cancela la alarma pendiente
                }
                ...
                let alarma_hilo = alarma.clone();
                thread::spawn(move || {
                    thread::sleep(Duration::from_millis(fin_ms - ahora_ms + 400));
                    if *alarma_hilo.lock().unwrap() != mi_gen {
                        return; // llegó un timer más nuevo: esta alarma ya no aplica
                    }
                    let _ = Command::new("osascript")
                        .arg("-e")
                        .arg("display notification \"¡Bloque terminado! El siguiente empieza solito\" with title \"Cuckoo ENARM\"")
                        .status();
                    for _ in 0..3 {
