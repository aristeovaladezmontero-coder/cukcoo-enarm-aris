...
fn inject_js() -> String {
  ...
  if (!window.__cuckooTickTimer) {{
    window.__cuckooTickTimer = setInterval(function () {{
      ...
      if (t !== window.__cuckooLast) {{
        window.__cuckooLast = t;
        if (window.__TAURI__ && window.__TAURI__.event) {{ window.__TAURI__.event.emit('cuckoo-tick', t); }}
      }}
    }}, 1000);
  }}
}})();"#,
